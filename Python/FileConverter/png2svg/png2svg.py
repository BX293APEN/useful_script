...
"""
    def __init__(self):
        ...
        self.minArea       = args.minArea


class PNG2SVG:
    """PNGをSVGアウトラインに変換するクラス。"""

    def __init__(self, inputPath, threshold, smooth, defaultStroke, defaultFill, svgWidth):
...
"""
    ...
    def output(self, outputPath: str = None) -> None:
        ...
        dest = outputPath or self.outputPath
...
if __name__ == "__main__":
    config    = ARGConfig()
    converter = PNG2SVG(config.inputPath, config.threshold, config.smooth, config.defaultStroke, config.defaultFill, config.svgWidth)
    converter.convert()
    converter.output()
