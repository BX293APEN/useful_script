...
"""
    def __init__(self):
        ...
        self.default_stroke = args.stroke
        self.default_fill   = args.fill
        self.svg_width      = args.width
...
class PNG2SVG:
    ...
    def _load_image(self) -> np.ndarray:
        ...
        img = cv2.imread(self.config.input_path, cv2.IMREAD_UNCHANGED)
        if img is None:
            sys.exit(f"[エラー] 画像を読み込めませんでした: {self.config.input_path}")
    ...
    def _binarize(self, img: np.ndarray) -> np.ndarray:
        ...
        _, binary = cv2.threshold(
            gray, self.config.threshold, 255, cv2.THRESH_BINARY_INV
        )
    ...
    def _collect_paths(self, binary: np.ndarray, scale: float) -> list[tuple]:
        ...
        for cnt, hier in zip(contours, hierarchy[0]):
            ...
            if area < self.config.min_area:
                skipped += 1
                continue
            path_d = self._contour_to_svg_path(cnt, self.config.smooth)
            if not path_d:
                continue
            is_hole = hier[3] != -1
    ...
    def _build_css(self, path_entries: list[tuple]) -> str:
        ...
        for idx, (area, _, is_hole) in enumerate(path_entries, start=1):
            ...
            f_val = "none" if is_hole else self.config.default_fill
            s_val = self.config.default_stroke
...
"""
    ...
    def convert(self) -> str:
        ...
        scale        = (self.config.svg_width / w) if self.config.svg_width and self.config.svg_width != w else 1.0
