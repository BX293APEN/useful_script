...
if [[ "$BOOT_MODE" == "uefi" ]]; then
    ...
    # --boot-directory にホスト側のフルパスを渡すと、GRUBコアに埋め込まれる
    # $prefix が /mnt/kali-repair/boot/grub になってしまい起動後に grub> で止まる。
    # --root-directory を指定することで grub-install が内部的に
    # /boot/grub を $prefix として埋め込むよう修正する。
    grub-install \
        --target=x86_64-efi \
...
# grub.cfg のコピー先について
#
# grub-install に --root-directory="${MNT}" を付けた場合、
# GRUBコアに埋め込まれる $prefix は /boot/grub になる。
# よって GRUB が起動時に探す grub.cfg は必ず:
#   (rootfsパーティション)/boot/grub/grub.cfg  ← $GRUB_CFG (生成済み)
#
# UEFIの --removable は EFIパーティション内に BOOTX64.EFI を置くが、
# その BOOTX64.EFI 自身が "(rootfsパーティション)/boot/grub/grub.cfg" を
# 探すので EFIパーティション側への grub.cfg コピーは不要。
#
# ただし一部のファームウェアが EFI/BOOT/grub.cfg を参照することがあるため
# EFIパーティション側にも念のためコピーしておく。
# ─────────────────────────────────────────────
if [[ "$BOOT_MODE" == "uefi" ]]; then
    # EFIパーティション内の grub.cfg（念のためのフォールバック）
    mkdir -p "${MNT}/boot/efi/EFI/kali"
    cp "$GRUB_CFG" "${MNT}/boot/efi/EFI/kali/grub.cfg"
    mkdir -p "${MNT}/boot/efi/EFI/BOOT"
    cp "$GRUB_CFG" "${MNT}/boot/efi/EFI/BOOT/grub.cfg"
    log_info "grub.cfg → 3箇所にコピーしました"
    log_info "  (rootfs)/boot/grub/grub.cfg         ← GRUBが参照するメインパス"
